...
/// Deterministic placement for a task index, derived from the MADT efficiency
/// classes: the lowest class fills first, in core order within the class, and
/// placement wraps across the placed cores once capacity is exhausted.
/// Pure function so the policy is host-testable without hardware.
pub fn place_task(topology: &CpuTopology, task_index: u32) -> Option<(u8, u32)> {
    let capacity: u32 = topology.classes().iter().map(|(_, count)| *count).sum();
    if capacity == 0 {
        return None;
    }
    let mut index = task_index % capacity;
    for &(class, count) in topology.classes() {
        if index < count {
...
mod tests {
    ...
    #[test]
    fn placement_fills_lowest_class_first_then_wraps() {
        let topology = crate::acpi::CpuTopology {
            classes: [
                (0, 2),
                (1, 2),
                (0, 0),
        ...
        assert_eq!(place_task(&topology, 0), Some((0, 0)));
        assert_eq!(place_task(&topology, 1), Some((0, 1)));
        assert_eq!(place_task(&topology, 2), Some((1, 0)));
        assert_eq!(place_task(&topology, 3), Some((1, 1)));
        assert_eq!(place_task(&topology, 4), Some((0, 0)), "wraps");
        assert_eq!(place_task(&topology, 7), Some((1, 1)), "wraps");
    }

    #[test]
    fn placement_of_empty_topology_is_unplaced() {
        let topology = crate::acpi::CpuTopology::default();
        assert_eq!(place_task(&topology, 0), None);
    }
